...
较低，那么本中断会处于Wait或者Suspend的位置，中断重入就无法发生。因为一旦发生重入，就
造成优先级反转，不符合中断系统的设置和硬件的实现。
...
模型声明与~\ref{subsubsec:basic_model_decl} 小节中描述的基本相似，见图
~\ref{fig:reentrant_model_decl}。
